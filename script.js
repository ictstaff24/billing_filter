...
function updateUploadBoxLoaded(fileName) {
  if (!dropZone || !uploadTitle || !uploadSubtitle) return;
  dropZone.classList.add('file-loaded');
  uploadTitle.textContent = 'Loaded file';
  uploadSubtitle.textContent = fileName;
  dropZone.title = `Loaded file: ${fileName}`;
}

function resetUploadBox() {
  if (!dropZone || !uploadTitle || !uploadSubtitle) return;
  dropZone.classList.remove('file-loaded');
  uploadTitle.textContent = 'Upload Excel file';
  uploadSubtitle.textContent = 'Click here or drag & drop .xlsx / .xls file';
...
function processExcelFile(file) {
  ...
  reader.onload = (e) => {
    try {
      ...
      updateUploadBoxLoaded(file.name);
